...
import { useAppointments, useFamilyAppointments, useAppointmentStats } from "@/features/services/appointmentService";
import { BookAppointmentDialog } from "./_components/BookAppointmentDialog";
import { AvailabilityDialog } from "@/features/pages/appointment/_components/AvailabilityDialog";
import { storeFunctions } from "@/store/authSlice";

const AppointmentPage = () => {
  const [searchTerm, setSearchTerm] = useState("");

  // Get current user role for role-based visibility
  ...
  const { data: appointments, isLoading: isLoadingAppointments } =
    useAppointments();
  const { data: familyAppointments, isLoading: isLoadingFamilyAppointments } =
  ...
  const appointmentStats: AppointmentStatPRops[] = [
    {
      ...
      appointmentStatus: [
        {
          label: "Completed",
          count: statsData?.yearly_appointments?.completed?.toString() || "0",
          color: "text-green-400",
        },
        {
          label: "Rescheduled",
          count: statsData?.yearly_appointments?.rescheduled?.toString() || "0",
          color: "text-yellow-400",
        },
        {
          label: "Cancelled",
          count: statsData?.yearly_appointments?.cancelled?.toString() || "0",
          color: "text-red-400",
        },
      ],
    },
    {
      ...
      period: statsData?.period?.month
        ? new Date(0, statsData.period.month - 1).toLocaleString("default", {
            month: "long",
          })
        : "Current Month",
      action: "Set Month",
      appointmentStatus: [
        {
          label: "Completed",
          count: statsData?.monthly_appointments?.completed?.toString() || "0",
          color: "text-green-400",
        },
        {
          label: "Rescheduled",
          count:
            statsData?.monthly_appointments?.rescheduled?.toString() || "0",
          color: "text-yellow-400",
        },
        {
          label: "Cancelled",
          count: statsData?.monthly_appointments?.cancelled?.toString() || "0",
          color: "text-red-400",
        },
      ],
    },
    {
      ...
      appointmentStatus: [
        {
          label: "Completed",
          count: statsData?.daily_appointments?.completed?.toString() || "0",
          color: "text-green-400",
        },
        {
          label: "Rescheduled",
          count: statsData?.daily_appointments?.rescheduled?.toString() || "0",
          color: "text-yellow-400",
        },
        {
          label: "Cancelled",
          count: statsData?.daily_appointments?.cancelled?.toString() || "0",
          color: "text-red-400",
        },
      ],
    },
  ...
  const filteredFamilyAppointments =
    familyAppointments?.filter(
      (appointment) =>
