...
import { usePatientsForAppointment, useCreateFamily } from "@/features/services/patientService";
...
const CreateFamilyDialog = ({ children }: { children: React.ReactNode }) => {
  ...
  const { data: patientsOptions = [], isLoading: loadingPatients } = usePatientsForAppointment();
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-center text-xl font-semibold">Create Family</DialogTitle>
        </DialogHeader>

        <div className="space-y-6">
          <p className="text-sm text-gray-500">Provide a family name, then add one or more patients as members. You can add or remove rows as needed.</p>
          ...
          <Forge {...{ control, onSubmit, ref: formRef }}>
                  ...
                  <div key={field.id ?? index} className="flex items-end gap-2">
                    <div className="flex-1">
                      <Forger
                        ...
                        placeholder={loadingPatients ? "Loading..." : "Select patient"}
            ...
            <div className="flex items-center justify-end gap-2 pt-4">
              ...
              <Button type="button" onClick={() => formRef.current?.onSubmit()} disabled={isPending}>
